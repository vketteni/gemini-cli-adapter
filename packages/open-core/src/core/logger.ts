...
// Provider-agnostic content interface
...
export class Logger {
  ...
  async saveCheckpoint(conversation: LoggableContent[], tag: string): Promise<void> {
    if (!this.initialized) {
      console.error(
  ...
  async loadCheckpoint(tag: string): Promise<LoggableContent[]> {
    if (!this.initialized) {
      console.error(
    ...
    try {
      ...
      return parsedContent as LoggableContent[];
    } catch (error) {
