...
import { CompatUtils } from '../types/compat.js';
...
import { convertGoogleResponse } from '../utils/responseConverter.js';
...
export class GeminiClient {
  ...
  async setTools(): Promise<void> {
    ...
    // Convert our provider-agnostic function declarations to Google format
    const googleDeclarations = toolDeclarations.map(decl => 
      CompatUtils.toGoogle.FunctionDeclaration(decl)
    );
    const tools: Tool[] = [{ functionDeclarations: googleDeclarations }];
  ...
  async generateJson(
    ...
    try {
      ...
      const text = getResponseText(convertGoogleResponse(result));
      if (!text) {
