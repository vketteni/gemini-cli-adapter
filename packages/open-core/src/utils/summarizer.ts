...
import { convertPartListUnion, convertGoogleResponse } from './responseConverter.js';
import { getResponseText as getProviderResponseText } from './generateContentResponseUtilities.js';
...
export const llmSummarizer: Summarizer = (result, geminiClient, abortSignal) =>
  summarizeToolOutput(
    partToString(convertPartListUnion(result.llmContent)),
    geminiClient,
    abortSignal,
  );
...
export async function summarizeToolOutput(
  ...
  try {
    ...
    return getProviderResponseText(convertGoogleResponse(parsedResponse)) || textToSummarize;
  } catch (error) {
